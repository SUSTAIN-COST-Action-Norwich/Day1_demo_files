...
# created by a script named "find_full-length_hits.pl" which parses a tblastn
# result file and extract the ORF information of top hits from each query using
# the HSP alignments in the blast result file.
# 
# USAGE: perl extract_orf.pl <lookup_file> <sequence_database_to_search> <output_fasta_file>
#
...
my $usage = "USAGE: perl extract_orf.pl \<lookup_file\> \<sequence_database_to_search\> \<output_fasta_file\>";

my ($lookup_file, $fasta_db, $out_file) = @ARGV;
unless ($lookup_file && $fasta_db && $out_file) {die "$usage\nAll files must be supplied\n"}

unless (-e $lookup_file) {die "Can't open $lookup_file: $!"}
...
my @lines = ();
open (IN, $lookup_file) || die "Can't open $lookup_file: $!";
my $query;
while (<IN>) {
    chomp;
    next if (/^\s*$/);
...
# Creates a name for writing a tab-separated sequence file with some annotations
my $tab_file = "";
if ($out_file =~ /^(\S+)\.\S+$/) {
	$tab_file = $1."\.tab";
} else {
...
# Processes each line in the input lookup file, extracts ID and other information, and 
# matches up with IDs from the sequence db file. When found a match, writes out the sequence
# and other information to two files: (i) fasta sequence file and (ii) tab-seperated data file
...
sub reverse_com {
	# Reverse-complements a nucleotide sequence from DNA
	my ($inputSeq) = shift;
...
sub find_orf {
	# Assumes that the first three nucleotide is the START codon "ATG",
	# looks at each triplet and collect in a sequence till it finds a STOP codon
	# or when it reaches the end of the input sequence
	# If it has found a STOP codon, returns the info back along with the ORF.
